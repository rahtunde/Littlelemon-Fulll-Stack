...
def bookings(request):
    if request.method == 'POST':
        ...
        # Check if a booking for the given date and slot already exists
        ...
        if not exist:
            ...
            # Booking already exists, return an error response
            return HttpResponse("{'error': 1}", content_type='application/json')

    # Retrieve date from the request or use today's date
    date = request.GET.get('date', datetime.today().date())
